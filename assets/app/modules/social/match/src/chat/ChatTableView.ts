import { _decorator, Component, instantiate, Node, Prefab, Pool, ScrollView, UITransform } from 'cc';
...
import { RevokedCell } from 'app/modules/social/chat/src/component/chatCell/RevokedCell';
...
export class ChatTableView extends XComponent {
    ...
    onLoad() {
        console.log("ChatTableView onLoad");
    ...
    getPool(message: Message, cell?: Node): Pool<Node> {
        ...
        if (!pool) {
            pool = new Pool<Node>(() => {
                ...
                console.log("this.textCell", this.textCell);
                switch (msgType) {
    ...
    private getContentSize() {
        let scrollView = this.tableView.getComponent(ScrollView);
        return scrollView?.content?.getComponent(UITransform).contentSize;
    }
    ...
    private createMessageCell(message: Message, index: number): Node {
        ...
        cell.getComponent(YogaFlex)?.updateLayout(this.getContentSize());
