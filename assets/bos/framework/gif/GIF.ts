import { ImageAsset, RenderTexture, SpriteFrame, Texture2D, assetManager, loader, log } from "cc";
import LZW from "./LZW";
...
class GIF {
    ...
    private async getTexture() {
        ...
        for (const frame of this._info.frames) {
            await this.decodeFrame2Texture(frame, index++);
        }
     ...
     * @param dataUrl
     */
    private dataUrl2SpriteFrame(dataUrl, width: number, height: number): Promise<SpriteFrame> {
     ...
     * @param h
     */
    private date2SpriteFrame(data, w, h): Promise<SpriteFrame> {
        // let texture = new RenderTexture();
        // let spriteFrame = new SpriteFrame();
        // texture.initWithData(data.data, Texture2D.PixelFormat.RGBA8888, w, h);
        // spriteFrame.texture = texture;
        // return spriteFrame;
        if (JSB) {
            return new Promise((resolve) => {
    ...
    private async decodeFrame2Texture(frame, index) {
        ...
        if (JSB) {
            ...
            this._spriteFrames[index] = await this.date2SpriteFrame(this._lastData, this._canvas.width, this._canvas.height);
        } else {
            //web平台
            let dataUrl = this.putImageDataWeb(imageData, frame)
            this._spriteFrames[index] = await this.dataUrl2SpriteFrame(dataUrl, this._canvas.width, this._canvas.height);
        }
...
class GIFCache {
    ...
    static getInstance() {
        if (!GIFCache.instance) {
            ...
            if (JSB) {
                ...
                assetManager.parser.register('.gif', async (file, options, onComplete) => {
                    let gif = new GIF();
    ...
    addItemFrame(key: any, frameData: GIFFrameData) {
        if (this.has(key) == true) {
            ...
            let gifCacheItem = { referenceCount: 0, type: FileType.GIF, frameData: frameData }
...
export { GIF, GIFCache };
export type { GIFFrameData, GIFCaheItem };
