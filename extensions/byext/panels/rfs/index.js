...
(function (exports) {
	...
	exports.renderToFile = function renderToFile (path, qrData, options, cb) {
	  ...
	  const stream = fs$1.createWriteStream(path);
...
function genQr(img, select_ip) {
    let ips = Editor.Network.queryIPList();
    let index = ips.indexOf(select_ip.value);
    ips.splice(index, 1);
    let sips = ips.join(",");
    sips = `${select_ip.value},${sips}`;
    let url = `http://${select_ip.value}:5678/launch.html?project=${Editor.Project.name}&ips=${sips}&port=5678`;
    console.log(url);
    qrcode.toDataURL(url, { errorCorrectionLevel: "H", type: "image/jpeg", width: 300, margin: 1 }).then(v => {
        img.src = v;
    }).catch(e => {
        console.log(e);
    });
}
function rfsBroadcast(content) {
    let msg = "";
    ...
    Editor.Message.send("byext", "rfs-broadcast", msg);
}
function query(parent, selector) {
    return parent.querySelector(selector);
}
module.exports = Editor.Panel.define({
    template: fs.readFileSync(path.join(__dirname, "main.html"), { encoding: "utf-8" }),
    style: fs.readFileSync(path.join(__dirname, "main.css"), { encoding: "utf-8" }),
    $: {
        main: "#main",
    },
    methods: {
        refreshUI: function () {
            return __awaiter(this, void 0, void 0, function* () {
                let main = this.$.main;
                let info = yield Editor.Message.request("byext", "query-rfs-info");
                let content = `服务状态: ${info.running}<br/>`;
                content = content + "clients: <br/><ul>";
                info.clients.forEach(client => {
                    content = content + `<li>${client}</li>`;
                });
                content = content + "</ul>";
                query(main, "#status").innerHTML = content;
            });
        }
    },
    close() {
        tools.log("panels.rfs.close");
    },
    ready() {
        tools.log("panels.rfs.ready");
        console.log("main", this.$.main);
        let main = this.$.main;
        query(main, "#btn_update").onclick = () => {
            rfsBroadcast({ event: "update" });
        };
        query(main, "#btn_clean").onclick = () => {
            rfsBroadcast({ event: "clean" });
        };
        query(main, "#btn_restart").onclick = () => {
            rfsBroadcast({ event: "restart" });
        };
        query(main, "#btn_restart_server").onclick = () => {
            Editor.Message.send("byext", "restart-rfsserver");
        };
        query(main, "#btn_open_build_panel").onclick = () => {
            Editor.Message.send("builder", "open");
        };
        let img_qr = query(main, "#img_qr");
        let select_ip = query(main, "#select_ip");
        select_ip.innerHTML = "";
        select_ip.onchange = () => {
            localStorage.setItem("byext.rfs.ip", select_ip.value);
            genQr(img_qr, select_ip);
        };
        let ips = Editor.Network.queryIPList();
        ips.forEach(ip => {
        ...
        select_ip.selectedIndex = 0;
        let last_ip = localStorage.getItem("byext.rfs.ip");
        if (last_ip) {
            let index = ips.indexOf(last_ip);
            if (index > -1) {
                select_ip.selectedIndex = index;
            }
        }
        genQr(img_qr, select_ip);
        this.refreshUI();
